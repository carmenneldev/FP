...
exports.AzureSQLAdapter = void 0;
const tslib_1 = require("tslib");
const sql = tslib_1.__importStar(require("mssql"));
const azure_sql_schema_1 = require("./azure-sql-schema");
class AzureSQLAdapter {
    ...
    async initializeSchema() {
        try {
            ...
            const statements = (0, azure_sql_schema_1.getSchemaStatements)();
            let tablesCreated = 0;
    ...
    async getFinancialAdvisorById(id) {
    ...
    async createFinancialAdvisor(advisor) {
    ...
    async createUserCredential(userCred) {
        const result = await this.query(`
    ...
    async createMaritalStatus(status) {
        const result = await this.query(`
    ...
    async createPreferredLanguage(language) {
        const result = await this.query(`
    ...
    // Bank Statements
    async getBankStatements(customerID) {
        return await this.query(`
    ...
    async createBankStatement(statement) {
        const result = await this.query(`
    ...
    // Profile Management
    async updateAdvisorProfile(userID, updateData) {
        // Whitelist allowed columns to prevent SQL injection
        const allowedColumns = [
            'firstName', 'surname', 'identityNumber', 'mobileNumber', 'emailAddress',
            'physicalAddress1', 'physicalAddress2', 'provinceID', 'postalCode',
