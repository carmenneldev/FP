...
isInvalid(controlName: string): boolean {
      ...
      this.http.get(`${environment.apiUrl}/Statements/${statementId}/status`, options)
        .subscribe({
          ...
          error: (error) => {
            ...
            if (error.status === 401) {
              this.uploadError = 'Authentication expired. Please log in again and try uploading.';
            } else if (error.status === 404) {
              this.uploadError = 'Statement not found. Please try uploading again.';
            } else {
              this.uploadError = `Failed to check processing status (${error.status || 'Network Error'})`;
            }
            clearInterval(pollInterval);
          }
        });
  ...
  uploadSelectedFile() {
    ...
    const token = localStorage.getItem('jwt_token');
    if (!token) {
      this.uploadError = 'Authentication required. Please log in and try again.';
      return;
    }
